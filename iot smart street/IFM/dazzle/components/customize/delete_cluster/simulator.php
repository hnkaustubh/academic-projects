	...
	<div class="table" style="padding-top: 50px;">
		<table border="1|8">
			...
			<?php
				...
				$clusterim = $_POST['cluster'];
				$nodeim = $_POST['node'];
				$sensorim = $_POST['sensor'];
				echo $clusterim;
				echo $nodeim;
				echo $sensorim;
				...
				if(isset($_POST['submit'])){
					...
					$newtime = strtotime("29 December 2002");
					...
					for($i=0; $i<1; $i++){
						...
						for($cluster=1; $cluster<=3; $cluster++){
							for($node=1; $node<=5; $node++){
								if($cluster == $clusterim && $node == $nodeim){
									if($sensorim == 'motion'){
										$motion = 'NULL';
										$light = $motion;
										$temperature = rand(42,68);
										$rain_guage = rand(2,25)/10;
									}	
									else if($sensorim == 'light'){
										$motion = rand(0,2);
										$light = 'NULL';
										$temperature = rand(42,68);
										$rain_guage = rand(2,25)/10;
									}
									else if($sensorim == 'temperature'){
										$motion = rand(0,2);
										$light = $motion;
										$temperature = 'NULL';
										$rain_guage = rand(2,25)/10;
									}
									else if($sensorim == 'rain_guage'){
										$motion = rand(0,2);
										$light = $motion;
										$temperature = rand(42,68);
										$rain_guage = 'NULL';
